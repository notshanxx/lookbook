...
button.addEventListener("click", () => {
  ...
  if (!inputISBN) {
    resultSpan.innerHTML =
      "inputted isbn seems <b style='color:red'> invalid</b>";
  ...
  }: ${dashedISBN} is ${checkISBN(inputISBN) ? "valid" : "invalid"}</mark>`;
...
function checkISBN(isbn) {
  ...
  console.log(sum % mod ? "Invalid ISBN" : "Valid ISBN");
...
function caculateISBN10(isbn) {
  ...
  formula += ` = ${sum} (mod 11) = ${sum % 11} ${
    sum % 11 === 0 ? "valid" : "invalid"
  }`;
